...
def app():
    ...
    def apply_prediction_on_dataX(data, model_selectbox, model_AraBert_all):
        predictions_list, probabilities_list = [], []
        
        for te in data:
            te = [str(te)]
            if model_selectbox == "AraBERT":
                prediction, raw_output = model_AraBert_all.predict(te) 
    ...
    def apply_prediction_on_data(data, model_selectbox, model_AraBert_all, model_MBERT):
        predictions_list, probabilities_list = [], []
        ...
        for te in data:
            ...
            if model_selectbox == "AraBERT":
                ...
                prediction, raw_output = model_MBERT.predict(te)
    ...
    model_AraBert_all = joblib.load('AraBert_all.pkl')
    model_MBERT = joblib.load('M_BERT.pkl')

    #      INTERFACE

    #    ********   SIDEBAR     *********
    st.sidebar.markdown('<br>', unsafe_allow_html=True)
    models_selectbox = st.sidebar.selectbox("Select Model", ["AraBERT", "Multilingual"])
    ...
    st.markdown('<style>p {direction: rtl;}</style>', unsafe_allow_html=True )
    st.markdown('<style>label {direction: rtl;}</style>', unsafe_allow_html=True )
    #st.markdown('<style>div {direction: rtl;}</style>', unsafe_allow_html=True )
    #st.markdown('<style>span {direction: rtl;}</style>', unsafe_allow_html=True )
    st.markdown('<style>.st-as {direction: rtl;}</style>', unsafe_allow_html=True )
    st.markdown('<style>.st-emotion-cache-16idsys {direction: rtl;}</style>', unsafe_allow_html=True )
    st.markdown('<style>.st-emotion-cache-1dtefog {direction: rtl;}</style>', unsafe_allow_html=True )
    st.markdown('<style>.st-emotion-cache-p5msec {direction: rtl;}</style>', unsafe_allow_html=True )
    
    st.markdown("<h2 style='text-align: right; color: #31333f;padding:0;margin:0;'>إكتشاف  النصوص التي تم إنشاؤها بواسطة الذكاء الاصطناعي</h2>", unsafe_allow_html=True)
    text_input = st.text_area("أدخل النص")
    #st.markdown("<p id='txtNote'>اضغط زر Enter عند الانتهاء من الكتابة</p>", unsafe_allow_html=True)
    uploaded_file = st.file_uploader("أو قم برفع ملف  (pdf)", type="pdf")

    # Submit button for text area or file upload
    btn_submit2check = st.button("إرسال")
    if btn_submit2check:
        ...
        if uploaded_file is not None or text_input != "":
            ...
            if uploaded_file is not None:
                ...
                data_list = divide_into_500_word_items(text)
            if len(data_list) > 0:
                apply_prediction_on_data(data_list, models_selectbox, model_AraBert_all, model_MBERT)
                
            else:
                st.markdown("<p style='text-align: right; color: #88001b;padding:0;margin:0;'>أدخل  نصًا للعمل عليه . . .</p>", unsafe_allow_html=True)
                
        else:
            st.markdown("<p style='text-align: right; color: #88001b;padding:0;margin:0;'>أدخل  نصًا للعمل عليه . . .</p>", unsafe_allow_html=True)

    with st.expander("قم بإضافة رأيك"):        
        feedback_radio = st.radio("هل كانت النتيجة صحيحة؟", ["نعم", "لا"], index=None)
        feedback_text = st.text_input("هل لديك أي تعليقات أخرى؟ (اختياري)")
        feedback_button = st.button("إرسال التعليقات")


        if feedback_button:
            if not feedback_radio and not feedback_text:
                st.write("لم يتم تحديد أي خيارات.")
            else:
                ...
                st.write("تم استلام التعليقات، شكرا لك على ملاحظاتك!")